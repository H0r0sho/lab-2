...
def next(path_to_csv: str, count: int) -> Optional[List[str]]:
    with open(path_to_csv + '/lab2/dataset.csv', 'r', encoding='utf-8') as csvfile:
...
def work_0(date: datetime.date,  path_to_csv: str=os.path.join("C:/", "Users", "79171", "PyCharmProjects")) -> str:
    ...
    with open(path_to_csv+'/lab2/dataset.csv', mode='r', encoding='utf-8') as csvfile:
...
def work_1(date: datetime.date,  path_to_csv: str=os.path.join("C:/", "Users", "79171", "PyCharmProjects")) -> str:
    "принимает данные, ищет их в файле соответствующего скрипта"
    with open(path_to_csv +'/lab2/X.csv', 'r', encoding='utf-8') as csvfile:
        file_reader = list(csv.reader(csvfile))
        for i in range(len(file_reader)):
            ...
            return None
    with open(path_to_csv+'/lab2/Y.csv', 'r', encoding='utf-8') as csvfile:
        file_reader = list(csv.reader(csvfile))
...
def work_2(date: datetime.date,  path_to_csv: str=os.path.join("C:/", "Users", "79171", "PyCharmProjects","lab2")) -> str:
    "принимает данные, ищет их в файле соответствующего скрипта"
    ways = os.listdir(path_to_csv+"/lab2")
    date = str(date)
    for i in range(len(ways)):
        if (ways[i][:4] == date[:4]):  # Ищем файл у которого аткой же год
            with open(path_to_csv +"/lab2/"+ ways[i], 'r', encoding='utf-8') as csvfile:
                file_reader = csv.reader(csvfile)
                for row in file_reader:
                    if (row[0] == date):
                        tmp = str(row[1])
...
def work_3(date: datetime.date,  path_to_csv: str=os.path.join("C:/", "Users", "79171", "PyCharmProjects")) -> str:
    ...
    for i in range(len(list1)):
        if (int(list1[i][11:13]) >= int(date[8:10]) >= int( list1[i][8:10])):  # если число находится в диапозоне недели то выводим его
            with open(path_to_csv + "/lab2/" + list1[i], 'r', encoding='utf-8') as csvfile:
                file_reader = csv.reader(csvfile)
                date = re.sub(r'[_]', '-', date)
                for row in file_reader:
                    if (row[0] == date):
